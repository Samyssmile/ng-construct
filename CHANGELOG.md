...
- **ci:** Deploy the demo app to GitHub Pages on every push to `main`
  (<https://samyssmile.github.io/ng-construct/>). Repository / docs only — no library change.

## [0.11.2] - 2026-06-29

### Changed

- **ci:** Add project automation — a **CI** workflow (`build:angular` → `ng test angular` →
  `build:demo` on every push/PR to `main`), a **Release** workflow that publishes to npm when a
  GitHub Release is published (idempotent: skips when `NPM_TOKEN` is unset or the version already
  exists), and **Dependabot** for weekly npm and GitHub Actions updates (Angular toolchain grouped).
  A CI status badge was added to the README. No library code changed.
